...
<html>
  <head>
    ...
    <title>投稿済みアイテムの編集 - Masterpiece(β) あなたの愛用品、教えてください。</title>
    ...
    <div class="container-fluid">
      ...
      <c:if test="${!noUser}" >
        ...
        <div class="span9">
          ...
          <div class="row-fluid">
            <div class="section">
            <form method="post" action="editItem" name="editForm">
              <input type="hidden" name="editTargetLoginID" value="<c:out value="${loginID_this_page}" />">
              <input type="hidden" name="editTargetItemKey" value="<c:out value="${editTarget['keyString']}" />">
              <div class="heading" id="edithead">
                <h3><i class="icon-pencil icon-white" ></i> 投稿済みアイテム「<a href="/user/<c:out value="${loginID_this_page}"/>/item/<c:out value="${editTarget['keyString']}" />" ><c:out value="${editTarget['name']}" />／<c:out value="${editTarget['categoryName']}" /></a>」の編集</h3>
              </div>
              <div class="content">
                <h3>１．登録カテゴリの変更</h3>
                <div class="controls" style="padding: 0 0 0 30px;">
                  <select id="editItemcategory" name="editItemcategory">
                    <option value="01" <c:if test="${editTarget['categoryCode'] == '01'}">selected</c:if>>コンピュータ</option>
                    <option value="02" <c:if test="${editTarget['categoryCode'] == '02'}">selected</c:if>>デジタルガジェット</option>
                    <option value="03" <c:if test="${editTarget['categoryCode'] == '03'}">selected</c:if>>携帯電話</option>
                    <option value="04" <c:if test="${editTarget['categoryCode'] == '04'}">selected</c:if>>電化製品</option>
                    <option value="05" <c:if test="${editTarget['categoryCode'] == '05'}">selected</c:if>>オーディオ機器</option>
                    <option value="06" <c:if test="${editTarget['categoryCode'] == '06'}">selected</c:if>>カメラ</option>
                    <option value="07" <c:if test="${editTarget['categoryCode'] == '07'}">selected</c:if>>生活雑貨</option>
                    <option value="08" <c:if test="${editTarget['categoryCode'] == '08'}">selected</c:if>>文房具</option>
                    <option value="09" <c:if test="${editTarget['categoryCode'] == '09'}">selected</c:if>>雑貨</option>
                    <option value="10" <c:if test="${editTarget['categoryCode'] == '10'}">selected</c:if>>CD/DVD類</option>
                    <option value="11" <c:if test="${editTarget['categoryCode'] == '11'}">selected</c:if>>書籍</option>
                    <option value="12" <c:if test="${editTarget['categoryCode'] == '12'}">selected</c:if>>おもちゃ/ホビー</option>
                    <option value="13" <c:if test="${editTarget['categoryCode'] == '13'}">selected</c:if>>模型/プラモデル</option>
                    <option value="14" <c:if test="${editTarget['categoryCode'] == '14'}">selected</c:if>>楽器</option>
                    <option value="15" <c:if test="${editTarget['categoryCode'] == '15'}">selected</c:if>>アート</option>
                    <option value="16" <c:if test="${editTarget['categoryCode'] == '16'}">selected</c:if>>アンティーク/コレクション</option>
                    <option value="17" <c:if test="${editTarget['categoryCode'] == '17'}">selected</c:if>>ファッション</option>
                    <option value="18" <c:if test="${editTarget['categoryCode'] == '18'}">selected</c:if>>アクセサリー</option>
                    <option value="19" <c:if test="${editTarget['categoryCode'] == '19'}">selected</c:if>>服飾雑貨/小物</option>
                    <option value="20" <c:if test="${editTarget['categoryCode'] == '20'}">selected</c:if>>家具/インテリア</option>
                    <option value="21" <c:if test="${editTarget['categoryCode'] == '21'}">selected</c:if>>時計</option>
                    <option value="22" <c:if test="${editTarget['categoryCode'] == '22'}">selected</c:if>>スポーツ用品</option>
                    <option value="23" <c:if test="${editTarget['categoryCode'] == '23'}">selected</c:if>>自動車/バイク</option>
                    <option value="24" <c:if test="${editTarget['categoryCode'] == '24'}">selected</c:if>>食品/飲料</option>
                    <option value="25" <c:if test="${editTarget['categoryCode'] == '25'}">selected</c:if>>ペット/生き物</option>
                    <option value="26" <c:if test="${editTarget['categoryCode'] == '26'}">selected</c:if>>植物</option>
                    <option value="99" <c:if test="${editTarget['categoryCode'] == '99'}">selected</c:if>>その他</option>
                  </select>
                </div>
                <h3>２．アイテム名称、コメント、参考URLの編集</h3>
                <div class="controls" style="padding: 0 0 0 30px;">
                  <label class="control-label" for="editItemName">アイテム名称</label>
                  <div class="controls">
                    <input type="text" class="input-xlarge" id="editItemName" name="editItemName" value="<c:out value="${editTarget['name']}" />"><span class="" id="edit-input-name-message" style="color:red;"></span>
                    <input type="hidden" id="editItemNameHidden" name="editItemNameHidden" value="">
                  </div>
                  <label class="control-label" for="editItemComment">コメント</label>
                  <div class="controls">
                    <textarea class="input-xxlarge" id="editItemComment" name="editItemComment" rows="6"><c:out value="${editTarget['replaceComments']}" /></textarea><span class="" id="edit-input-comment-message" style="color:red;"></span>
                    <input type="hidden" id="editItemCommentHidden" name="editItemCommentHidden" value="">
                  </div>
                  <label class="control-label" for="editItemUrl">参考URL</label>
                  <div class="controls">
                    <input type="text" class="input-xlarge" id="editItemUrl" name="editItemUrl" value="<c:out value="${editTarget['url']}" />"><span class="" id="edit-input-url-message" style="color:red;"></span>
                  </div>
                  <a href="#edithead" class="btn btn-primary" style="vertical-align:middle;display:table-cell;" id="editButton">編集する</a>

                  <div id="editModal" class="modal hide fade">
                    <div class="modal-header">
                      <a class="close" data-dismiss="modal" >&times;</a>
                      <h3>投稿済みアイテム「<c:out value="${editTarget['name']}" />／<c:out value="${editTarget['categoryName']}" />」の編集</h3>
                    </div>
                    <div class="modal-body">
                      <p>下記の内容で登録内容の上書きを行います。</p>
                      <br>
                      <h4>登録カテゴリ</h4>
                      ...
                      <div id="editModalLoadingZone" align="center"></div>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-danger" id="editButtonOnModal" type="submit">編集する</button>
