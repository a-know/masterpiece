    ...
    <div class="container-fluid">
      ...
      <c:if test="${!noUser}" >
        ...
        <div class="span9">
          ...
          <c:if test="${itemList_size == 0}" var="noItem" />
          <c:if test="${noItem}" >
          ...
          <c:forEach var="item" items="${itemList_top}" varStatus="status">
            <div class="section" style="clear:both;">
              <div class="heading" id="posthead">
                ...
                <c:if test="${noUrl}" >
                <h3><a href="/user/<c:out value="${loginID_this_page}"/>/item/<c:out value="${itemKeyList[status.index]}" />" style="text-decoration: underline; color: white;"><c:out value="${item.name}" />（<c:out value="${categoryNameList[status.index]}" />）</a></h3>
                </c:if>
                <c:if test="${!noUrl}" >
                <h3><a href="/user/<c:out value="${loginID_this_page}"/>/item/<c:out value="${itemKeyList[status.index]}" />" style="text-decoration: underline; color: white;"><c:out value="${item.name}" />（<c:out value="${categoryNameList[status.index]}" />）</a><a href="<c:out value="${item.url}" />" target="_blank">【参考リンク】</a></h3>
                </c:if>
              </div>
              <div class="content">
