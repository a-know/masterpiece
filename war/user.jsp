  ...
  <script>
    ...
    <div class="container-fluid">
      ...
      <c:if test="${sessionScope.twitter_release_success == '1'}">
        <div class="alert alert-success">
          <a class="close" data-dismiss="alert">&times;</a>
          Twitterとの連携の解除が完了しました。
        </div>
      </c:if>

          <div style="width:100%;height:300px;overflow-y: hidden;margin:20px auto;position: relative;" class="userPanel">
            <div class="am-container" id="am-container" style="width:100%;">
              <c:if test="${fn:length(montageImageList) == 0}" var="noItem" />
              <c:if test="${noItem}" >
              <img src="../images/masterpiece_sample01.jpg"></img>
              <img src="../images/masterpiece_sample02.jpg"></img>
              <img src="../images/masterpiece_sample03.jpg"></img>
              <img src="../images/masterpiece_sample04.jpg"></img>
              </c:if>
              <c:if test="${!noItem}" >
              <c:forEach var="item" items="${montageImageList}" varStatus="status">
              <img src="<c:out value="${item}" />"></img>
              </c:forEach>
              </c:if>
            </div>
            <div id="overlay" class="headcontent">
              <div class="headinner">
                <i class="icon-tag icon-white" ></i> <span style="color:white;"><c:out value="${loginID_this_page}" />'s Masterpiece<br>
                <c:if test="${user.url == ''}"  var="noUserUrl" />
                <c:if test="${noUserUrl}" >
                id:<c:out value="${loginID_this_page}" /><br>
                </c:if>
                <c:if test="${!noUserUrl}" >
                id:<c:out value="${loginID_this_page}" /><a href="<c:out value="${user.url}" />" target="_blank">【URL】</a><br>
                </c:if>
                <c:out value="${user.postCount}" /> Masterpieces<br>
                <span style='color:#ffcc00;'>★</span>×<c:out value="${user.starCount}" /><br>
                <c:out value="${user.introduction}" /><br>
                </span>
                <a href="http://b.hatena.ne.jp/entry/http://master-piece.appspot.com/user/<c:out value="${loginID_this_page}" />" class="hatena-bookmark-button" data-hatena-bookmark-title="<c:out value="${loginID_this_page}" />'s Masterpiece" data-hatena-bookmark-layout="standard" title="このエントリーをはてなブックマークに追加"><img src="http://b.st-hatena.com/images/entry-button/button-only.gif" alt="このエントリーをはてなブックマークに追加" width="20" height="20" style="border: none;" /></a>
                <a href="https://twitter.com/share" class="twitter-share-button" data-url="http://master-piece.appspot.com/user/<c:out value="${loginID_this_page}" />" data-text="<c:out value="${loginID_this_page}" />'s Masterpiece" data-lang="ja">ツイート</a><br>
                <div class="fb-like" data-href="http://master-piece.appspot.com/user/<c:out value="${loginID_this_page}" />" data-send="false" data-layout="button_count" data-width="350" data-show-faces="false"></div>
              </div>
            </div>
          </div>
